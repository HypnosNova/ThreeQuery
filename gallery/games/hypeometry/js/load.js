function createLoadingScene() {
	...
	function init() {
		...
		new THREE.TTFLoader().load('font/loadFont.ttf', function(json) {
...
//			group.add(explosionAnimation);
			...
			for(var i = 0; i < textAnimationData.info.length; i++) {
				...
				var pos = textAnimationData.info[i].boundingBox.getCenter();
	...
	function ExplosionAnimation(data) {
		...
		for(var index = 0; index < letterCount; index++) {
			...
			letterBox.getSize(glyphSize);
			letterBox.getCenter(glyphCenter);
