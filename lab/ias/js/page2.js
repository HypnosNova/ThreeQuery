function createWorld2() {
	function init() {
		...
		world.camera.position.set(0, 0, 140);
		...
		new THREE.ImageLoader().load('assets/img/photo.jpg', function(image) {
		...
		new THREE.ImageLoader().load('assets/img/ias.jpg', function(image) {
		...
		btn.element.scale.x /= 3;
		btn.element.scale.y /= 3;
		btn.element.position.y = -36;
		world.scene.add(btn.element);

		new TWEEN.Tween(btn.element.material).to({
			opacity: 1
		}).delay(3000).start().onComplete(function() {
			...
			btn.element.onClick = function() {
				...
				btn.element.onClick=false;
				$$.rayCasterEventReceivers = [];
				world.rayCasterEventReceivers = [];
				var worldB = createWorld3();
					console.log("aaa");
				var transition = new $$.Transition(worldB, {}, $$.Loader.RESOURCE.textures['assets/transition/transition7.png']);
