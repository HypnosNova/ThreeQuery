function createWorld3() {
	...
	world.showText = function() {
		...
		var text1 = new $$.Txt("1.WebGL项目的架构思路", {
			color: "#ffff33",
			fontSize: 60,
			width: 1000,
			height: 120,
		...
		text1.element.scale.x /= 2.5;
		text1.element.scale.y /= 2.5;
		text1.element.position.y = 14;
		world.scene.add(text1.element);

		var text2 = new $$.Txt("2.WebGL的性能优化方法", {
			color: "#ffff33",
			fontSize: 60,
			width: 1000,
			height: 120,
		...
		text2.element.scale.x /= 2.5;
		text2.element.scale.y /= 2.5;
		text2.element.position.y = 2;
		world.scene.add(text2.element);

		var text3 = new $$.Txt("3.VR，AR领域的应用场景", {
			color: "#ffff33",
			fontSize: 60,
			width: 1000,
			height: 120,
		...
		text3.element.scale.x /= 2.5;
		text3.element.scale.y /= 2.5;
		text3.element.position.y = -10;
		world.scene.add(text3.element);

		var text4 = new $$.Txt("4.融合更多样的交互方式", {
			color: "#ffff33",
			fontSize: 60,
			width: 1000,
			height: 120,
		...
		text4.element.scale.x /= 2.5;
		text4.element.scale.y /= 2.5;
		text4.element.position.y = -22;
		world.scene.add(text4.element);
		...
		new TWEEN.Tween(title.element.material).to({
			...
			new TWEEN.Tween(text1.element.material).to({
				opacity: 1
			}).start().onComplete(function() {
				new TWEEN.Tween(text2.element.material).to({
					opacity: 1
				}).start().onComplete(function() {
					new TWEEN.Tween(text3.element.material).to({
						opacity: 1
					}).start().onComplete(function() {
						new TWEEN.Tween(text4.element.material).to({
							opacity: 1
						}).start().onComplete(function() {
							new TWEEN.Tween(btn.element.material).to({
								...
								btn.element.onClick = function() {
									...
									var worldB = createWorld4();
									var transition = new $$.Transition(worldB, {}, $$.Loader.RESOURCE.textures['assets/transition/transition6.png']);
	...
	function Animation(modelGeometry) {
		...
		var material = new BAS.StandardAnimationMaterial({
			...
			uniformValues: {
				diffuse: new THREE.Color(0xff9a46),
				roughness: 0.8,
				metalness: 0.8,
				opacity: 0.8
			},
