...
function loadStarSurfaceTextures(){
	if( sunTexture === undefined ){
		sunTexture = loader.load( "images/sun_surface.png", undefined, setLoadMessage("Igniting solar plasma") );
		sunTexture.anisotropy = maxAniso;
		sunTexture.wrapS = sunTexture.wrapT = THREE.RepeatWrapping;
	}

	if( sunColorLookupTexture === undefined ){
		sunColorLookupTexture = loader.load( "images/star_colorshift.png" );
	}

	if( solarflareTexture === undefined ){
		solarflareTexture = loader.load( "images/solarflare.png", undefined, setLoadMessage("Distributing solar flares")	 );
	}

	if( sunHaloTexture === undefined ){
		sunHaloTexture = loader.load( "images/sun_halo.png", undefined, setLoadMessage("Calculating coronal mass") );
	}

	if( sunHaloColorTexture === undefined ){
		sunHaloColorTexture = loader.load( "images/halo_colorshift.png" );
	}

	if( sunCoronaTexture === undefined ){
		sunCoronaTexture = loader.load( "images/corona.png", undefined, setLoadMessage("Projecting coronal ejecta") );
	}
}
	...
	var sunHaloMaterial = new THREE.ShaderMaterial(
		{
			...
			depthTest: 		false,
			depthWrite: 	false,
			defaultAttributeValues :{color: 0xffffff},
